...
- 2026-05-02: Moved deterministic result-entry generation and resolved/pushed view-state shaping into extension domain helpers. Store actions now call `buildExtensionViewState`, and the later Phase 7 cleanup removed the temporary service compatibility re-exports.
...
- 2026-05-03: Moved app update version comparison and installer asset selection from `src/services/appUpdater.js` into `src/domains/settings/appUpdatePresentation.js`. The updater service is now focused on GitHub release fetch, Tauri update download/reveal commands, external release links, and download progress events; the later Phase 12 cleanup removed the temporary helper re-exports.
...
- 2026-05-03: Moved LaTeX preview source-selection matching from `src/services/latex/previewSync.js` into `src/domains/latex/latexPreviewSelection.js`. The preview sync service now focuses on SyncTeX target resolution, editor view waiting, and source reveal side effects, dropping below the over-150-line service review threshold; the later Phase 12 cleanup removed the temporary selection-helper re-export.
- 2026-05-03: Split workspace preference presentation and DOM side effects out of `src/services/workspacePreferences.js`. Pure defaults, font presets, font encoding, PDF viewer display normalization, and font-stack helpers now live in `src/domains/settings/workspacePreferencePresentation.js`; DOM font variables live in `src/services/workspaceFonts.js`; theme class/listener side effects live in `src/services/workspaceTheme.js`. `workspacePreferences.js` dropped below the over-150-line service review threshold.
...
- 2026-05-03: Removed the remaining workspace preference compatibility export hub. Workspace theme helpers are imported from `src/services/workspaceTheme.js`, font DOM helpers from `src/services/workspaceFonts.js`, and preference presentation constants/normalizers from `src/domains/settings/workspacePreferencePresentation.js`; `src/services/workspacePreferences.js` now keeps only Rust preference/workbench invokes plus system-font listing.
