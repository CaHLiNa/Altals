...
pub(crate) fn sync_research_task_context_for_thread(
    workspace_path: &str,
    runtime_thread_id: &str,
    ...
    let mut task = tasks[index].clone();
    let normalized_evidence_ids = normalize_vec(Some(evidence_ids));
    if !normalized_evidence_ids.is_empty() {
    ...
    let normalized_reference_ids = normalize_vec(Some(reference_ids));
    if !normalized_reference_ids.is_empty() {
        task.reference_ids = normalized_reference_ids;
    }
    task.updated_at = now_ms();
