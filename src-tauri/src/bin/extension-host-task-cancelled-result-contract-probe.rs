...
fn unique_temp_dir() -> Result<PathBuf, String> {
    ...
    let root = std::env::temp_dir().join(format!(
        "scribeflow-extension-host-task-cancelled-contract-{now}"
    ));
...
fn run_probe(probe_root: &Path, home_root: &Path) -> Result<(), String> {
    ...
    let persisted_task = task_entry(home_root, &task_id)?.ok_or_else(|| {
        format!("Expected persisted task record for cancelled probe task {task_id}")
    })?;
