...
fn unique_temp_dir() -> Result<PathBuf, String> {
    ...
    let root = std::env::temp_dir()
        .join(format!("scribeflow-extension-host-task-cancelled-contract-{now}"));
...
fn build_probe_extension(workspace_root: &Path) -> Result<PathBuf, String> {
    let extension_root = workspace_root
        .join(".scribeflow")
        .join("extensions")
        .join("example-task-cancelled-contract-extension");
    let manifest_path = extension_root.join("package.json");
    let entry_path = extension_root.join("dist").join("extension.js");
    let manifest = json!({
        "name": "example-task-cancelled-contract-extension",
        "displayName": "Example Task Cancelled Result Contract Extension",
        "version": "0.1.0",
        "type": "module",
        "main": "./dist/extension.js",
        "activationEvents": [
            "onCommand:exampleTaskCancelledContractExtension.inspect"
        ],
        "contributes": {
            "commands": [
                {
                    "command": "exampleTaskCancelledContractExtension.inspect",
                    "title": "Inspect Task Cancelled Result Contract"
                }
            ]
        },
        "permissions": {
...
export async function activate(context) {
  context.commands.registerCommand('exampleTaskCancelledContractExtension.inspect', async () => {
    return {
      ...
      artifacts: [
        {
          ...
          path: `${context.workspace.rootPath}/cancel.log`,
        },
      ],
      outputs: [
        {
...
"#;
    ...
    write_file(
        ...
        &serde_json::to_string_pretty(&manifest).map_err(|error| {
            format!("Failed to serialize task cancelled probe manifest: {error}")
        })?,
    )?;
...
fn run_probe(probe_root: &Path, home_root: &Path) -> Result<(), String> {
    ...
    fs::create_dir_all(&workspace_root)
        .map_err(|error| format!("Failed to create cancelled probe workspace dir: {error}"))?;
    fs::create_dir_all(&global_config_dir)
        .map_err(|error| format!("Failed to create cancelled probe global config dir: {error}"))?;
    ...
    let activate = extension_host_activate_by_id_for_probe(
        ...
        &workspace_root_text,
        "example-task-cancelled-contract-extension",
        "onCommand:exampleTaskCancelledContractExtension.inspect",
    )?;
    if !activate.activated {
        return Err(format!(
            "Task cancelled contract extension did not activate: {}",
            activate.reason
        ));
    }
    ...
    let task = extension_task_create_command_for_probe(
        "example-task-cancelled-contract-extension",
        "exampleTaskCancelledContractExtension.inspect",
        "workspace",
    ...
    let result = extension_command_record_result_for_probe(
        ...
        ExtensionHostCapabilityResult {
            accepted: true,
            message: "translation cancelled by user".to_string(),
            ...
            artifacts: vec![ExtensionArtifact {
                ...
                capability: "exampleTaskCancelledContractExtension.inspect".to_string(),
                kind: "log".to_string(),
                media_type: "text/plain".to_string(),
                path: workspace_root.join("cancel.log").to_string_lossy().to_string(),
    ...
    let persisted_task = task_entry(home_root, &task_id)?
        .ok_or_else(|| format!("Expected persisted task record for cancelled probe task {task_id}"))?;

    if result.task.outputs.first().map(|entry| entry.text.as_str()) != Some("cancelled by user") {
        return Err(format!(
            "Task cancelled result lost cancel output summary: {:?}",
            result.task.outputs
        ));
    }
    if persisted_task
    ...
    {
        return Err(format!(
            "Expected persisted cancelled task state, got {persisted_task}"
        ));
    }
    if persisted_task
        .get("progress")
    ...
    {
        return Err(format!(
            "Expected cancelled task progress label to preserve host label, got {persisted_task}"
        ));
    }
    if persisted_task
        .get("artifacts")
    ...
    {
        return Err(format!(
            "Expected cancelled task to keep one artifact, got {persisted_task}"
        ));
    }
    if persisted_task
        .get("outputs")
    ...
    {
        return Err(format!(
            "Expected cancelled task to keep one output, got {persisted_task}"
        ));
    }
    if persisted_task
        .get("outputs")
    ...
    {
        return Err(format!(
            "Expected cancelled task output title to survive persistence, got {persisted_task}"
        ));
    }
    if persisted_task
        .get("error")
    ...
    {
        return Err(format!(
            "Expected cancelled task error to remain empty, got {persisted_task}"
        ));
    }
    ...
    println!(
        ...
        .map_err(|error| format!("Failed to serialize task cancelled contract summary: {error}"))?
    );
