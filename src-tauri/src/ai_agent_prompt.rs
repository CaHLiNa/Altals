...
mod tests {
    ...
    #[tokio::test]
    async fn build_prompt_uses_agent_first_shape_for_agent_runtime() {
        let response = ai_agent_build_prompt(AiAgentPromptParams {
            skill: json!({ "id": "workspace-agent", "kind": "built-in-action" }),
            context_bundle: json!({
            ...
            user_instruction: "Fix the failing build.".to_string(),
            conversation: vec![json!({ "role": "user", "content": "Earlier request" })],
            altals_skills: vec![json!({
            ...
            enabled_tool_ids: vec![
                ...
                "write-workspace-file".to_string(),
                "open-workspace-file".to_string(),
            ],
            runtime_intent: "agent".to_string(),
        })
        .await
        .expect("prompt");
