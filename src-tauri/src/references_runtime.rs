...
pub async fn references_find_duplicate(params: ReferenceDuplicateParams) -> Result<Value, String> {
    Ok(find_duplicate_reference_internal(&params.existing, &params.candidate).unwrap_or(Value::Null))
}
...
pub async fn references_import_pdf(params: ReferencePdfImportParams) -> Result<Value, String> {
    ...
    Ok(build_fallback_pdf_reference(normalized_path, &first_text, &metadata))
}
...
pub async fn references_write_bib_file(params: ReferenceBibFileParams) -> Result<String, String> {
    ...
    let bib_path = format!(
        "{}/references.bib",
        tex_path.rsplit_once('/').map(|(dir, _)| dir).unwrap_or_default()
    );
    let header =
...
#[cfg(test)]
mod tests {
    use super::{find_duplicate_reference_internal, merge_imported_references_internal, parse_csl_metadata};
    use serde_json::json;

    #[test]
    fn duplicate_detection_matches_by_citation_key() {
        ...
        let duplicate = find_duplicate_reference_internal(&existing, &candidate).unwrap();
        assert_eq!(duplicate.get("id").and_then(|value| value.as_str()), Some("a"));
    }

    #[test]
    fn merge_imported_skips_duplicates() {
        let existing = vec![json!({
            "id": "a",
            "citationKey": "smith2024",
            "identifier": "",
        ...
        let merged = merge_imported_references_internal(&existing, &imported);
        assert_eq!(merged.len(), 2);
    }

    #[test]
    fn csl_metadata_parser_extracts_title_and_category() {
        let style = parse_csl_metadata(
            r#"<style><info><title>My Style</title><id>https://www.zotero.org/styles/my-style</id><category citation-format="numeric" /></info></style>"#,
            "my-style.csl",
        );
