...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        std::env::temp_dir().join(format!("altals-references-{label}-{}", Uuid::new_v4()))
    }

    #[test]
    fn normalize_snapshot_removes_legacy_entries_and_backfills_fields() {
        let snapshot = normalize_snapshot(&json!({
            "references": [
                {
                    "id": "ref-1",
        ...
        assert_eq!(trim_string(references[0].get("id")), "user-1");
        assert_eq!(
            trim_string(references[0].get("typeKey")),
            "conference-paper"
        );
        assert!(bool_value(references[0].get("hasPdf")));
    }

    #[test]
    fn load_or_create_snapshot_migrates_legacy_project_library() {
        let global_root = temp_dir("global");
        let legacy_root = temp_dir("legacy-project");
        fs::create_dir_all(global_root.join("references")).unwrap();
        fs::create_dir_all(legacy_root.join("references")).unwrap();

        fs::write(
            global_root.join("references").join("library.json"),
        ...
        .unwrap();
        fs::write(
            legacy_root.join("references").join("library.json"),
            serde_json::to_string(&json!({
                "version": 2,
                "citationStyle": "apa",
        ...
        let snapshot = load_or_create_snapshot(&ReferenceLibraryReadParams {
            global_config_dir: global_root.to_string_lossy().to_string(),
            legacy_workspace_data_dir: String::new(),
            legacy_project_root: legacy_root.to_string_lossy().to_string(),
        })
        .unwrap();
        ...
        let references = snapshot
            ...
            .unwrap();
        assert_eq!(references.len(), 1);
        assert_eq!(trim_string(references[0].get("id")), "legacy-ref-1");
        assert!(bool_value(snapshot.get("legacyMigrationComplete")));
    ...
    #[test]
    fn load_or_create_snapshot_skips_legacy_import_after_migration_completed() {
        let global_root = temp_dir("global-complete");
        let legacy_root = temp_dir("legacy-complete");
        fs::create_dir_all(global_root.join("references")).unwrap();
        fs::create_dir_all(legacy_root.join("references")).unwrap();
        ...
        fs::write(
            ...
            serde_json::to_string(&json!({
                ...
                "collections": [{ "key": "control-theory", "label": "控制理论" }],
                "tags": [],
                "references": [{ "id": "legacy-ref-1", "title": "Legacy Paper" }]
            }))
            .unwrap(),
        )
        .unwrap();
