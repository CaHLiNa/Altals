...
#[cfg(test)]
mod tests {
    use super::document_workflow_reconcile;
    use super::DocumentWorkflowReconcileParams;
    use serde_json::json;

    #[tokio::test]
    async fn markdown_reconcile_prefers_workspace_preview() {
        let result = document_workflow_reconcile(DocumentWorkflowReconcileParams {
            active_file: "/workspace/chapter.md".to_string(),
            active_pane_id: "pane-source".to_string(),
            pane_tree: json!({
                "type": "leaf",
                "id": "pane-source",
                "tabs": ["/workspace/chapter.md"],
                "activeTab": "/workspace/chapter.md"
            }),
            trigger: "manual".to_string(),
            preview_prefs: json!({ "markdown": { "preferredPreview": "html" } }),
            detached_sources: json!({}),
    ...
    #[tokio::test]
    async fn latex_reconcile_stays_source_only() {
        let result = document_workflow_reconcile(DocumentWorkflowReconcileParams {
            active_file: "/workspace/main.tex".to_string(),
            active_pane_id: "pane-source".to_string(),
            pane_tree: json!({
                "type": "leaf",
                "id": "pane-source",
    ...
    #[tokio::test]
    async fn legacy_preview_binding_is_preserved_when_present() {
        let source_path = "/workspace/chapter.md";
        let preview_path = format!("preview:{source_path}");
        let result = document_workflow_reconcile(DocumentWorkflowReconcileParams {
            active_file: source_path.to_string(),
            active_pane_id: "pane-source".to_string(),
            pane_tree: json!({
                "type": "split",
                "direction": "vertical",
            ...
            trigger: "editor-pane-sync".to_string(),
            preview_prefs: json!({ "markdown": { "preferredPreview": "html" } }),
            detached_sources: json!({}),
