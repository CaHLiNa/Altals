...
- `Phase 1` 已完成：并行后端骨架已接入，默认仍为 `pdfjs`
- `Phase 2` 已完成：workspace 偏好与基本 view state 恢复已接入
- `Phase 3` 已完成：`saveAsCopy -> writePdfArtifactBase64` 保存链路已接入
- `Phase 4` 已完成：`forward SyncTeX` 已接到 EmbedPDF backend
- `Phase 5` 已完成：`reverse SyncTeX` 已接到 EmbedPDF backend
- `文本选择 / 复制` 已补齐：接入 `SelectionPlugin + InteractionManager`
- `搜索 UI` 已补齐：接入 `SearchPlugin` 与自定义 toolbar / 键盘导航
- `真实运行态验证` 已完成：前端构建、lint 通过，桌面交互由用户完成手工验证且未报告问题
- `Phase 6` 已进入独立切换 phase：默认 backend 已切到 `embedpdf`，`pdfjs` 改为显式回滚 backend

## 已完成实施范围

已完成：

1. 添加 EmbedPDF Headless 依赖与 `plugin-export`
2. 新增 backend selector
3. 新增 `PdfEmbedSurface.vue` 与 `PdfEmbedDocumentSurface.vue`
4. 在 `PdfArtifactPreview.vue` 内根据 selector 选择 backend
5. 接入 zoom / spread 偏好与基本 view state 恢复
6. 接入保存回写
7. 接入 `forward / reverse SyncTeX`
8. 保持默认仍为 `pdfjs`
9. 接入文本选择 / 复制、搜索 UI 与键盘快捷键
10. 完成一轮真实运行态 smoke test 收口
11. 新建独立默认切换 phase，并把默认 backend 切到 `embedpdf`
...
- `npm run build`
- `npm run lint`
- 用户手工验证 `EmbedPDF` 路线下的文本选择 / 复制、搜索 UI 与桌面交互，结果正常
- `VITE_SCRIBEFLOW_PDF_BACKEND=pdfjs npm run build`

后续在切换默认 backend 前建议补：

- 继续观察切换后真实工作区中的 crash / render / SyncTeX 回归
- 到回滚窗口结束前评估是否可以删除 `pdfjs` override
...
- `latex` 与普通 `pdf` 路径均可走 EmbedPDF backend
- 默认 backend 已切为 `embedpdf`
- `pdfjs` 仍可通过 override 回滚
- 前端构建与 lint 通过
- 文本选择 / 复制、搜索 UI 与运行态验证缺口已闭合
- `Phase 6` 已进入执行态，并已有回滚策略与保留窗口
