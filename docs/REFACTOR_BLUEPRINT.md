...
A narrow persistent left rail for `Project` / `Library` / `AI` navigation has now also landed as a shell-level component, replacing the previous header-level primary-surface switcher without reintroducing broader orchestration into `App.vue`; the left-sidebar toggle affordance now lives as one persistent control in the global header, tracking the live left-sidebar edge while expanded and snapping back to the mac traffic-light safe slot or the left-rail anchor when collapsed instead of duplicating a second toggle in the top-right chrome. The mirrored right-sidebar toggle now uses right-edge geometry directly while expanded, so non-fullscreen macOS chrome does not inherit the left-side padding offset. That shell chrome is now driven by one shared slot-based geometry helper instead of separate ad hoc `left`/`right` offset formulas inside `Header.vue`, so windowed-vs-fullscreen positioning stays on the same coordinate model. The header chrome no longer updates those positions through four separate inline style objects either; the shared shell geometry now feeds one set of CSS custom properties on the header root, so drag-time updates stay on a single coordinate path.

That shell chrome is no longer tied to one project-only left-sidebar mode set. The left sidebar is now a surface-aware host: the workspace surface now switches between `Project files` / `References`, the library surface owns its own outer-shell sidebar panels for `Library views` and `Library tags`, and the AI surface has now been simplified down to one shell-level recent-chat sidebar instead of carrying a separate current-context mode. In practice that means the library and AI workbenches no longer carry their own internal left rails; their panel navigation has moved up into the shared shell, while contextual `Outline` inspection for the workspace surface lives on the mirrored right inspector instead of being duplicated on both sides.

Within the workspace file tree itself, the old top-right workspace menu button has now been removed. Workspace-level folder actions now live in a dedicated bottom action row, keeping the file-tree header focused on navigation/search/create controls instead of mixing those with workspace switching actions.
...
The resizable left sidebar also now resolves its minimum width against the largest shell button group across `workspace` / `library` / `ai`, instead of shrinking to a smaller per-surface minimum when the current surface exposes fewer header buttons. Both shell sidebars now stay mounted while the workspace shell is open instead of being torn down on every toggle, and drag-time width updates are now batched to animation frames while the shell enters an explicit `resizing` state that disables hover transitions and pointer interaction inside the sidebar content. Outside resize, the shell now uses short width/slide/fade transitions for sidebar open/close so the chrome reads as deliberate motion instead of mount/unmount flicker.
