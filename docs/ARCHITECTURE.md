...
The Rust backend is also the landing zone for the active native editor stack:
...
- `src-tauri/src/native_editor_runtime.rs` is the stateful Tauri-side session owner for the helper process, native interaction inspection, and helper-backed session snapshots
- `src/components/editor/NativePrimaryTextSurface.vue` is the active workbench text surface host, while editor interaction policy is progressively concentrated into `src/domains/editor/nativePrimarySurfaceRuntime.js` plus Rust-backed inspection and planning commands
- save and persistence paths materialize current document text from helper-backed runtime state rather than from a CodeMirror-owned document model
- native editor host and bridge crates live under `src-tauri/crates/*`, not inside the Vue shell
- editor workflow policy stays above the core, while low-level text mechanics and interaction analysis move into Rust over time

Do not reintroduce user-facing experimental editor controls, legacy Web editor fallbacks, or placeholder editor surfaces unless explicitly approved.
