...
export async function buildWorkspaceMeta(workspacePath) {
  ...
  const openFiles = [...editorStore.allOpenFiles].filter(f => !isChatTab(f) && !isNewTab(f) && !isAiLauncher(f) && !isLibraryPath(f))
  ...
  if (activeTab && (isChatTab(activeTab) || isNewTab(activeTab) || isAiLauncher(activeTab) || isLibraryPath(activeTab))) {
