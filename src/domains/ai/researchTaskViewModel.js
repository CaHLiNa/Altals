...
export function normalizeResearchTask(task = null) {
  ...
  return {
    ...
    artifactIds: Array.isArray(task.artifactIds) ? task.artifactIds.filter(Boolean) : [],
    evidenceIds: Array.isArray(task.evidenceIds) ? task.evidenceIds.filter(Boolean) : [],
    referenceIds: Array.isArray(task.referenceIds) ? task.referenceIds.filter(Boolean) : [],
